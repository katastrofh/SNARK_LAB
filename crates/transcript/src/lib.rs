...
impl core::fmt::Debug for MerlinTranscript {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter
            .debug_struct("MerlinTranscript")
...
impl<F: PrimeField> ProofTranscript<F> for MerlinTranscript {
    ...
    fn append_field_element(&mut self, label: &'static [u8], element: &F) {
        let encoded_len = F::MODULUS_BIT_SIZE.div_ceil(8) as usize;
...
mod tests {
    ...
    #[test]
    fn field_elements_use_fixed_width_encoding() {
        #[derive(Default)]
        struct RecordingTranscript(Vec<u8>);

        impl ProofTranscript<Fr> for RecordingTranscript {
            fn append_domain_separator(&mut self, _: &'static [u8]) {}
            fn append_field_element(&mut self, _: &'static [u8], element: &Fr) {
                let encoded_len = Fr::MODULUS_BIT_SIZE.div_ceil(8) as usize;
                let mut bytes = element.into_bigint().to_bytes_le();
                bytes.resize(encoded_len, 0);
                self.0 = bytes;
            }
            fn append_bytes(&mut self, _: &'static [u8], _: &[u8]) {}
            fn append_u64(&mut self, _: &'static [u8], _: u64) {}
            fn challenge_scalar(&mut self, _: &'static [u8]) -> Fr {
                Fr::ZERO
            }
        }

        let mut transcript = RecordingTranscript::default();
        transcript.append_field_element(b"element", &Fr::from(1_u64));
        assert_eq!(transcript.0.len(), 32);
    }
}
