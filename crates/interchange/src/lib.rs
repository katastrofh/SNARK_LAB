...
pub const MAX_ORACLE_EVALUATIONS: usize = 1 << 16;
...
impl fmt::Display for TranscriptError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InputTooLarge => write!(f, "transcript exceeds configured resource limits"),
...
pub fn verify_transcript(transcript: &Transcript) -> Result<(), TranscriptError> {
    ...
    if length > MAX_ORACLE_EVALUATIONS {
        return Err(TranscriptError::InputTooLarge);
    }
...
pub fn parse_and_verify(json: &str) -> Result<Transcript, ParseOrVerifyError> {
    if json.len() > MAX_TRANSCRIPT_BYTES {
        return Err(ParseOrVerifyError::Transcript(
            TranscriptError::InputTooLarge,
        ));
    }
...
mod tests {
    ...
    fn rejects_oversized_input_before_parsing() {
        let oversized = " ".repeat(MAX_TRANSCRIPT_BYTES + 1);
        assert!(matches!(
            parse_and_verify(&oversized),
            Err(ParseOrVerifyError::Transcript(
                TranscriptError::InputTooLarge
            ))
        ));
    }
