...
impl core::fmt::Display for VerifyError {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::WrongRoundCount => {
                formatter.write_str("proof round count does not match the statement")
            }
            Self::RoundDoesNotMatchClaim { round } => {
                write!(formatter, "round {round} does not match the current claim")
            }
            Self::FinalEvaluationMismatch => {
                formatter.write_str("final claim does not match the oracle evaluation")
            }
        }
    }
}
