...
Open `http://localhost:5173`. The redesigned workbench includes protocol traces, reduction diagrams, soundness explanations, explicit Rust/browser security boundaries, responsive layouts, keyboard-accessible controls, and reduced-motion support. Browser arithmetic is educational F₉₇ and is not the Rust cryptographic path. It has no telemetry or external font dependency.
...
- **Bounded untrusted input.** Educational transcript files and browser tables have explicit resource limits and canonical-value checks.
- **Hardened deployment defaults.** The static app ships CSP, referrer, permissions, framing, and cross-origin isolation headers for hosts that support `_headers` files.
...
- [Threat model and production-readiness gates](docs/threat-model.md)
...
```bash
./scripts/check-pr-ready.sh origin/main
```

The script checks Rust formatting, Clippy, tests, a clean reproducible web install/build, dependency advisories, textual diffs, binary payloads, working-tree cleanliness, and a non-destructive `git merge-tree` against the supplied target. To inspect a GitHub ZIP without overwriting this checkout, run `./scripts/inspect-github-archive.sh <https-url>`; it rejects path traversal and symlinks, extracts into a temporary directory, and prints a comparison. This checkout currently has no Git remote or archive URL configured, so a real upstream could not be fetched during this change.
